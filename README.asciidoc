:src-dir: app/src/main/groovy/io/github/babeloff/gremlin
...
== Build a GraalVM binary

The gradle toolchain detection does not work with Chocolatey, so we have to go with `jabba`.
The following installs `jabba`.
[source,powershell]
----
...
----
The following looks at all the packages available in `jabba`.
[source,powershell]
----
jabba ls-remote
----
This is the version of graalvm that we are using.
[source,powershell]
----
jabba install graalvm-ce-java11@21.1.0
----

We also need a compiler.
[source,powershell]
----
...
In order for the compiler to work we need to have an appropriate environment.
